...
- 页面分为 3 个独立白色圆角区域：
  ...
  - 左侧在线候选区
  - 右侧本地角色库区
- 页面底色保持浅灰，列表区之间只保留卡片间距，不再使用中间灰色分隔块。
...
- 顶部操作区按钮顺序：`录入所选角色` 在左；`编辑作品`、`删除所选角色` 固定在最右。
...
- 控件位置：位于“本地角色库”卡片底部，右对齐显示。
- 每个角色先计算剩余可补数量：
  - `remaining = per_character_limit - current_reference_count`
  - `remaining <= 0` 直接跳过
- 在线抓图与身份过滤都使用 `remaining` 作为上限，避免无效下载和超量补图。
- 批量补图为纯补充模式：不会删除本地已有参考图；只有当本地参考图数量小于上限时，才按差额补充新图。
- 进度条默认隐藏，仅在批量补图运行时显示；数字显示在进度条外侧右上角，进度条本体不显示数字。
